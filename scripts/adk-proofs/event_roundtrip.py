"""FACT 8: an ADK Event carrying a pending confirmation survives a JSON round
trip, so the pending call can be stored in Firestore and replayed next turn."""
import asyncio
import json
...
def update_note(note_id: str, summary: str) -> dict:
    executed.append(note_id); return {"ok": True}
...
class ScriptedLlm(BaseLlm):
    ...
    async def generate_content_async(self, r, stream=False) -> AsyncGenerator[LlmResponse]:
        self.seen.append(r); yield self.turns[min(len(self.seen)-1, len(self.turns)-1)]

def fc(n, a): return LlmResponse(content=types.Content(role="model", parts=[
    types.Part(function_call=types.FunctionCall(id="fc-1", name=n, args=a))]))
def txt(t): return LlmResponse(content=types.Content(role="model", parts=[types.Part(text=t)]))
...
async def main():
    ...
    # Serialize every event as memex would into Firestore, then rebuild.
    wire = [json.loads(e.model_dump_json(exclude_none=True)) for e in stored.events]
    print("FACT 8 events serialized:", len(wire))
    print("FACT 8 total JSON bytes:", len(json.dumps(wire)))
    revived = [Event.model_validate(d) for d in wire]

    # Replay into a brand-new service, as a fresh HTTP request would.
    fresh = InMemorySessionService()
    s2 = await fresh.create_session(app_name="memex", user_id="u")
    for e in revived:
        await fresh.append_event(s2, e)
    ...
    async for _ in Runner(agent=agent(llm2), app_name="memex", session_service=fresh).run_async(
        ...
        pass
    print("FACT 8 tool executed after round-trip + replay:", executed)
